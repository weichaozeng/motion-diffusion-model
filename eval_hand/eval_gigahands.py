        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(dataloader), desc="Generating Eval Samples"):
                ...
                for bs_i in range(batch['x_pose'].shape[0]):
                    print(f'x_trans: {batch["x_trans"][bs_i].shape}')
                    print(f'x_root_trans: {batch["x_root_trans"][bs_i].shape}')
                    entry = {
                        ...
                        'gt_trans': batch['x_trans'][bs_i].permute(1, 0) + batch['x_root_trans'][bs_i].unsqueeze(0),
